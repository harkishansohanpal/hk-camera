...
export default function Viewer() {
  ...
  function handleFullscreen() {
    ...
    // iOS: webkitEnterFullscreen kills WebRTC tracks on exit, so use CSS fullscreen instead
    if (isIOS) {
  ...
  // Clean up CSS fullscreen on unmount
  useEffect(() => { return () => setCssFullscreen(false); }, []);

  const isBad = status === 'error' || status === 'disconnected';

  return (
    <div className={`${cssFullscreen ? 'fixed inset-0 z-[9999]' : 'absolute inset-0'} flex flex-col bg-black`}
      style={{ paddingTop: 'env(safe-area-inset-top)' }}>
      <nav className={`z-20 flex items-center justify-between px-3 py-1.5 bg-black/80 backdrop-blur-xl border-b border-white/10 ${cssFullscreen ? 'hidden' : ''}`}
        ...
        <button onClick={handleFullscreen} className="w-11 h-11 flex items-center justify-center text-white/60 hover:text-white rounded-xl transition-colors" title="Fullscreen">
          {cssFullscreen ? <X size={22} /> : <Maximize2 size={22} />}
        </button>
      </nav>

      {cssFullscreen && (
        <button onClick={() => setCssFullscreen(false)}
