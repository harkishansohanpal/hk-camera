      ...
      <div className="flex flex-col gap-4 mb-6">
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <select
            ...
            className="input py-2.5 text-sm flex-1 sm:flex-none sm:w-auto"
          >
            <option value="">All cameras</option>
            {cameras.map((c) => <option key={c.id} value={c.id}>{c.name}</option>)}
          </select>
          <select value={filter} onChange={(e) => setFilter(e.target.value)} className="input py-2.5 text-sm flex-1 sm:flex-none sm:w-auto">
        ...
        <div className="flex flex-col gap-3">
          ...
          <div className="card">
            <label className="flex items-center gap-3 cursor-pointer">
              ...
              <span className="text-slate-300 font-medium">Select All ({filtered.length})</span>
            </label>
          </div>
          ...
          {filtered.map((rec) => (
            <div key={rec.id} className="card flex items-center gap-4">
              {/* Checkbox */}
              <label className="w-10 h-10 flex items-center justify-center -ml-2 -my-2 cursor-pointer flex-shrink-0">
                <input
                  type="checkbox"
              ...
              <button
                ...
                className="w-14 h-10 bg-slate-700 hover:bg-hk-500/20 rounded-lg flex items-center justify-center flex-shrink-0 transition-colors"
              >
                <Play size={18} className="text-hk-400" />
              </button>
              ...
              <div className="flex gap-2 flex-shrink-0">
                <a href={rec.url} download className="p-2.5 text-slate-400 hover:text-hk-400 transition-colors" title="Download">
                  <Download size={15} />
                </a>
                <button onClick={() => handleDelete(rec.id)} className="p-2.5 text-slate-400 hover:text-red-400 transition-colors" title="Delete">
                  <Trash2 size={15} />
                </button>
              </div>
            </div>
