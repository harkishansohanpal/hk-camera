  ...
  return (
    <div className="max-w-4xl mx-auto space-y-4 animate-fade-in">
      ...
      <div className="card p-3 flex flex-wrap items-center gap-2">
        ...
        <input type="datetime-local" value={timeRange.from} onChange={(e) => setTimeRange((t) => ({ ...t, from: e.target.value }))}
          className="input py-1.5 text-xs w-44" />
        <span className="text-[10px] text-slate-600">To</span>
        <input type="datetime-local" value={timeRange.to} onChange={(e) => setTimeRange((t) => ({ ...t, to: e.target.value }))}
          className="input py-1.5 text-xs w-44" />
      </div>

      <form onSubmit={handleAnalyze} className="flex gap-2">
        <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="e.g., Why did the viewer disconnect? Any recent errors?"
        ...
        {answer && (
          <button type="button" onClick={clear} className="btn-secondary text-sm px-3">
            <X size={14} /> Clear
          </button>
        )}
      </form>
      ...
      {answer && !loading && (
        <div className="card p-4">
          <div className="flex items-center gap-2 mb-3 text-[11px] text-slate-500">
            ...
            {filters.tag && (
              <span className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded bg-slate-800 text-slate-400 font-mono text-[10px]">{filters.tag}</span>
            )}
          </div>
