  ...
  return (
    <div className="relative">
      <button onClick={() => setIsOpen(!isOpen)}
        className="flex flex-col items-center justify-center gap-0.5 w-11 h-11 rounded-xl transition-colors text-slate-400 hover:text-white hover:bg-white/5">
          ...
          <div className="space-y-4">
            <div>
              <div className="flex justify-between items-center mb-1">
                ...
                <span className="text-xs text-slate-400">{settings.exposure.toFixed(1)} EV</span>
              </div>
              <input type="range" min="-3" max="3" step="0.1" value={settings.exposure}
                ...
                className="w-full h-1.5 bg-slate-700 rounded-full appearance-none cursor-pointer accent-hk-500" />
            </div>
            <div>
              <div className="flex justify-between items-center mb-1">
                <label className="text-xs font-medium text-white">Focus</label>
                <span className="text-xs text-slate-400">{settings.focus}%</span>
              </div>
              <input type="range" min="0" max="100" value={settings.focus}
                onChange={(e) => onControlChange('focus', parseInt(e.target.value))}
                className="w-full h-1.5 bg-slate-700 rounded-full appearance-none cursor-pointer accent-hk-500" />
              <p className="text-[10px] text-slate-500 mt-1">0 = near, 100 = far</p>
            </div>
            <div>
              ...
              <select value={settings.whiteBalance} onChange={(e) => onControlChange('whiteBalance', e.target.value)}
                className="w-full bg-slate-700 text-white rounded-lg px-2.5 py-1.5 text-sm border border-slate-600">
            ...
            <div>
              <div className="flex justify-between items-center mb-1">
                ...
                <span className="text-xs text-slate-400">{settings.iso}</span>
              </div>
            ...
            <button onClick={() => { onReset(); setIsOpen(false); }}
              className="w-full px-3 py-2 bg-slate-700 hover:bg-slate-600 text-white text-xs rounded-xl transition-colors">
              Reset
            </button>
          </div>
        </div>
